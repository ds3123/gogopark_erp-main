...
import { useCreate_Beauty } from "hooks/react-query/service/useCreateBeauty" ;
...
// @ 新增 _ 美容單
export const useEffect_Create_Beauty = () => { 
    ...
    const create_Beauty_Fun    = useCreate_Beauty() ;      // 美容單 
    ...
    // 執行 _ 新增函式
    const create_Beauty = async( data : any , shop_Id? : string ) => {

        // 轉換為資料表欄位
        const obj_Arr      = columns_Covert_Beauty( data ) ;
        const obj_Customer = obj_Arr[ 0 ] ;  // 客戶
        const obj_Pet      = obj_Arr[ 1 ] ;  // 寵物
        const obj_Beauty   = obj_Arr[ 2 ] ;  // 美容單
